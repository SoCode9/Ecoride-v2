...
final class CarpoolDisplay
{
    ...
    public static function one(
        ...
        $depCity       = is_array($c) ? (string)($c['departure_city'] ?? '') : (string)$c->getDepartureCity();
        $arrCity       = is_array($c) ? (string)($c['arrival_city'] ?? '')   : (string)$c->getArrivalCity();
        ...
        $driverPseudo  = is_array($c) ? ($c['driver_pseudo'] ?? null)        : null;
        $driverPhoto   = is_array($c) ? ($c['driver_photo'] ?? null)         : null;
        $carElectric   = is_array($c) ? (bool)($c['car_electric'] ?? 0)      : false;
        ...
        $seatsOffered   = $carId ? (int)$carRepo->getSeatsOfferedByCar($carId) : 0;
        $passengers     = $id ? (int)$resRepo->countPassengers($id) : 0;
        $seatsAvail     = max(0, $seatsOffered - $passengers);
        $seatsLabel     = $seatsAvail <= 1 ? "$seatsAvail place" : "$seatsAvail places";

        // --- formatage affichage ---
        $depTime = $depTimeRaw ? DateFormatter::time($depTimeRaw) : '';
        $arrTime = $arrTimeRaw ? DateFormatter::time($arrTimeRaw) : '';
        $dateUi  = $dateRaw    ? DateFormatter::short($dateRaw)   : '';
        ...
        if ($withActions) {
            ...
            $canParticipate = ($isGuest || $isPassenger) && !$isOwner && $status === 'not started' && $seatsAvail > 0;
        ...
        return [
            ...
            'driver_id'      => htmlspecialchars($driverId, ENT_QUOTES, 'UTF-8'),
            'driver_pseudo'  => $driverPseudo ? htmlspecialchars($driverPseudo, ENT_QUOTES, 'UTF-8') : '',
    ...
    public static function many(
        ...
        return array_map(
            fn(array $r) => self::one($r, $currentUserId, $driverService, $resRepo, $carRepo, $userRepo, $router, false),
            $rows
        );
    }
